<?php $__env->startSection('title', 'Cover Templates'); ?>
...
<div class="layout-px-spacing">
    <div class="middle-content container-xxl p-0">
        ...
        <div class="row layout-top-spacing mb-4 align-items-center">
            <div class="col-md-6">
                <h3 class="m-0 fw-bold">Cover Templates</h3>
            </div>
            <div class="col-md-6 text-md-end mt-3 mt-md-0 page-title-action">
                <a href="<?php echo e(route('cover-templates.create')); ?>" class="btn btn-primary">
        ...
        <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
            <div class="widget-content widget-content-area br-8">
                <div class="table-responsive">
                    <table id="templates-table" class="table dt-table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th width="5%">No</th>
                                <th>Cover Preview</th>
...
$(document).ready(function() {
    ...
    const table = $('#templates-table').DataTable({
        ...
        ajax: {
            url: "<?php echo e(route('cover-templates.index')); ?>",
            data: function(d) { d.status = $('#statusFilter').val(); }
        },
        columns: [
            { data: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'cover_image_path', name: 'cover_image_path', orderable: false, searchable: false },
            { data: 'name', name: 'name' },
    ...
    $('#resetFilters').on('click', function() { $('#statusFilter').val(''); table.draw(); });

    // Handle Delete
    $(document).on('click', '.delete-record', function(e) {
        e.preventDefault();
        const url = $(this).data('url');
        const templateName = $(this).closest('tr').find('td:eq(2)').text();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: url, type: 'DELETE', data: {_token: '<?php echo e(csrf_token()); ?>'},
                    ...
                    error: function(xhr) {
                        Toast.fire({ icon: 'error', title: xhr.responseJSON.message || 'Server Error' });
                    }
                });
            }
        });
    ...
    $(document).on('click', '.toggle-status', function(e) {
        e.preventDefault();
        $.post($(this).data('url'), {_token: '<?php echo e(csrf_token()); ?>'}, function(res) {
            if(res.success) {
                table.draw();
                Toast.fire({ icon: 'success', title: res.message });
            }
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/kaptensa/salman/resources/views/master/cover-templates/index.blade.php ENDPATH**/ ?>