{{-- resources/views/dashboard/health-check.blade.php --}}
@extends('layouts.app')

@section('title', 'Health Check Dashboard')

{{-- STYLES: CSS Dependencies & Custom Styles --}}
@push('styles')
...
<div class="row layout-top-spacing">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="text-muted mb-0" style="font-size: 14px;">
                    ...
                    <span class="text-dark fw-bold">
                        {{ \Carbon\Carbon::parse(request('start_date', now()))->format('d M Y') }}
                        -
            ...
            <div>
                <span class="badge badge-modern bg-soft-primary border border-primary-subtle">
                    <x-feather-icon name="activity" class="me-1" style="width: 14px; height: 14px;" />
                    Live Monitoring
                </span>
            </div>
        </div>
    ...
    <div class="col-12 layout-spacing">
        <div class="filter-card">
            <form method="GET" action="{{ route('dashboard.health-check') }}" id="filterForm">
                <div class="row align-items-end">

                    {{-- Filter Perusahaan --}}
                    @if(auth()->user()->hasAnyRole(['nurse', 'super-admin']))
                    ...
                    <div class="col-md-4 mb-3 mb-md-0 d-none d-md-block">
                        <div class="d-flex align-items-center text-muted">
                            <x-feather-icon name="filter" class="me-2" />
        ...
        <div class="widget widget-t-sales-widget widget-m-sales h-100">
            <div class="media">
                <div class="icon ml-2 text-primary bg-soft-primary p-2 rounded">
                    <x-feather-icon name="anchor" />
                </div>
                <div class="media-body">
                    <p class="widget-text mb-1">Total Kapal</p>
                    <p class="widget-numeric-value fs-4 mb-0">{{ $totalVessels }}</p>
                </div>
            </div>
            <p class="widget-total-stats mt-2 text-muted" style="font-size: 12px;">{{ $activeVessels }} Kapal Beroperasi</p>
        </div>
    </div>
        ...
        <div class="widget widget-t-sales-widget widget-m-orders h-100">
            <div class="media">
                <div class="icon ml-2 text-info bg-soft-primary p-2 rounded" style="color: #4361ee; background: #e0e7ff;">
                    <x-feather-icon name="users" />
                </div>
                <div class="media-body">
                    <p class="widget-text mb-1">Total Kru</p>
                    <p class="widget-numeric-value fs-4 mb-0">{{ $totalCrew }}</p>
                </div>
            </div>
            <p class="widget-total-stats mt-2 text-muted" style="font-size: 12px;">{{ $activeCrew }} Kru Aktif</p>
        </div>
    </div>
        ...
        <div class="widget widget-t-sales-widget widget-m-customers h-100">
            <div class="media">
                <div class="icon ml-2 text-success bg-soft-success p-2 rounded">
                    <x-feather-icon name="check-circle" />
                </div>
                <div class="media-body">
                    <p class="widget-text mb-1">Kepatuhan DCU</p>
                    <p class="widget-numeric-value fs-4 mb-0">{{ number_format($complianceRate, 1) }}%</p>
                </div>
            </div>
            <p class="widget-total-stats mt-2 text-muted" style="font-size: 12px;">{{ $totalChecks }} Laporan Masuk</p>
        </div>
    </div>
    ...
    <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6 layout-spacing">
        <div class="widget widget-t-sales-widget widget-m-customers h-100">
            <div class="media">
                <div class="icon ml-2 text-success bg-soft-success p-2 rounded">
                    <x-feather-icon name="clipboard" />
                </div>
                <div class="media-body">
                    <p class="widget-text mb-1">Kepatuhan MCU</p>
                    <p class="widget-numeric-value fs-4 mb-0">{{ number_format($mcuComplianceRate, 1) }}%</p>
                </div>
            </div>
            <p class="widget-total-stats mt-2 text-muted" style="font-size: 12px;">{{ $validMcuCount }} / {{ $activeCrew }} Kru Valid</p>
        </div>
    </div>
        ...
        <div class="widget widget-t-sales-widget widget-m-income h-100">
            <div class="media">
                <div class="icon ml-2 text-danger bg-soft-danger p-2 rounded">
                    <x-feather-icon name="activity" />
                </div>
                <div class="media-body">
                    <p class="widget-text mb-1">Isu Kesehatan</p>
                    <p class="widget-numeric-value fs-4 mb-0 text-danger">{{ $healthIssues }}</p>
                </div>
            </div>
            <p class="widget-total-stats mt-2 text-muted" style="font-size: 12px;">{{ $requiresFollowUp }} Perlu Tindak Lanjut</p>
        </div>
    </div>

    <div class="col-12 d-none d-xl-block"></div>

    {{-- CHART ROW 1: Trend & Summary --}}
        ...
        <div class="widget widget-chart-one h-100">
            <div class="widget-heading d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="badge bg-light text-dark border">
                    {{ \Carbon\Carbon::parse(request('start_date', now()))->format('d M') }} -
                ...
                <div class="order-summary">
                    ...
                    <div class="summary-list mb-4">
                        <div class="w-icon bg-soft-success text-success border border-success border-opacity-25">
                            <x-feather-icon name="check" />
                        </div>
                        <div class="w-summary-details">
                            <div class="w-summary-info">
                                <h6 class="mb-0 fw-bold">Sehat / Normal</h6>
                                <p class="summary-count fw-bold text-success">{{ $normalHealthCount }}</p>
                            </div>
                            <div class="w-summary-stats mt-2">
                                <div class="progress" style="height: 6px;">
                                    <div class="progress-bar bg-success" role="progressbar"
                                         style="width: {{ $normalHealthPercentage }}%"></div>
                                </div>
                                <small class="text-muted mt-1 d-block">{{ number_format($normalHealthPercentage, 1) }}% dari total</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="summary-list mb-4">
                        ...
                        <div class="w-summary-details">
                            <div class="w-summary-info">
                                ...
                                <p class="summary-count fw-bold text-warning">{{ $attentionHealthCount }}</p>
                            </div>
                            <div class="w-summary-stats mt-2">
                                <div class="progress" style="height: 6px;">
                                    <div class="progress-bar bg-warning" role="progressbar"
                                         style="width: {{ $attentionHealthPercentage }}%"></div>
                                </div>
                                <small class="text-muted mt-1 d-block">{{ number_format($attentionHealthPercentage, 1) }}% dari total</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="summary-list">
                        ...
                        <div class="w-summary-details">
                            <div class="w-summary-info">
                                ...
                                <p class="summary-count fw-bold text-danger">{{ $criticalHealthCount }}</p>
                            </div>
                            <div class="w-summary-stats mt-2">
                                <div class="progress" style="height: 6px;">
                                    <div class="progress-bar bg-danger" role="progressbar"
                                         style="width: {{ $criticalHealthPercentage }}%"></div>
                                </div>
                                <small class="text-muted mt-1 d-block">{{ number_format($criticalHealthPercentage, 1) }}% dari total</small>
                            </div>
                        </div>
                    </div>
            ...
            <div class="widget-content">
                <div class="table-responsive">
                    <table class="table table-hover" id="vesselComplianceTable">
                        ...
                        <tbody>
                            @forelse($vesselCompliance as $vessel)
                            <tr class="vessel-row" data-vessel-id="{{ $vessel['id'] }}" data-vessel-name="{{ $vessel['name'] }}">
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="avatar avatar-sm me-2 bg-light-primary text-primary rounded-circle d-flex align-items-center justify-content-center" style="width:32px; height:32px;">
                                            <x-feather-icon name="anchor" style="width: 16px; height: 16px;" />
                                        </div>
                                        <span class="fw-bold text-dark">{{ $vessel['name'] }}</span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    <span class="badge bg-light text-dark border">{{ number_format($vessel['total_crew']) }}</span>
                                </td>
                                <td class="text-center">
                                    <span class="fw-bold text-dark">{{ number_format($vessel['checks_count']) }}</span>
                                    <span class="text-muted"> / {{ number_format($vessel['target_checks']) }}</span>
                                </td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="progress flex-grow-1 me-3" style="height: 8px; background-color: #e2e8f0;">
                                            ...
                                            <div class="progress-bar {{ $barClass }}" role="progressbar"
                                                 style="width: {{ min($vessel['compliance_rate'], 100) }}%; border-radius: 4px;"
                                                 aria-valuenow="{{ $vessel['compliance_rate'] }}"
                                                 aria-valuemin="0"
                                                 aria-valuemax="100">
                                            </div>
                                        </div>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    <div class="d-flex flex-column align-items-center">
                                        <x-feather-icon name="inbox" style="width: 48px; height: 48px;" class="text-muted mb-3" />
            ...
            <div class="modal-header">
                <div>
                    <h5 class="modal-title mb-1" id="vesselDetailModalLabel">
                        <x-feather-icon name="anchor" class="me-2" style="width: 20px; height: 20px;" />
...
<script>
    $(document).ready(function() {
        ...
        $('.vessel-row').on('click', function() {
            ...
            const startDate = $('#start_date').val() || '{{ request("start_date", now()->format("Y-m-d")) }}';
            const endDate = $('#end_date').val() || '{{ request("end_date", now()->format("Y-m-d")) }}';
            ...
            // Fetch vessel detail
            $.ajax({
                url: '{{ route("dashboard.health-check.vessel-detail") }}',
                method: 'GET',
                data: {
                    vessel_id: vesselId,
                ...
                error: function(xhr) {
                    $('#vesselDetailContent').html(`
                        <div class="alert alert-danger text-center">
                            <x-feather-icon name="alert-circle" class="mb-2" style="width: 32px; height: 32px;" />
                        ...
                        <div class="timeline-item ${statusClass}">
                            ...
                            <div class="row g-3 mt-2">
                                <div class="col-4">
                                    <div class="p-3 bg-light rounded">
                                        <div class="d-flex align-items-center mb-1">
                                            <x-feather-icon name="users" style="width: 16px; height: 16px;" class="text-muted me-2" />
                                ...
                                <div class="col-4">
                                    <div class="p-3 bg-light rounded">
                                        <div class="d-flex align-items-center mb-1">
                                            <x-feather-icon name="check-circle" style="width: 16px; height: 16px;" class="text-success me-2" />
                                ...
                                <div class="col-4">
                                    <div class="p-3 bg-light rounded">
                                        <div class="d-flex align-items-center mb-1">
                                            <x-feather-icon name="x-circle" style="width: 16px; height: 16px;" class="text-danger me-2" />
    ...
    var trendOptions = {
        ...
        series: [{
            name: 'Kondisi Normal',
            data: @json($complianceTrendData['healthy'])
        }, {
            name: 'Ada Gejala/Sakit',
            data: @json($complianceTrendData['warning'])
        }],
        xaxis: {
            categories: @json($complianceTrendData['labels']),
    ...
    bpOpts.series = @json(array_values($bloodPressureDistribution));
    bpOpts.labels = @json(array_keys($bloodPressureDistribution));
    ...
    pulseOpts.series = @json(array_values($pulseRateDistribution));
    pulseOpts.labels = @json(array_keys($pulseRateDistribution));
    ...
    tempOpts.series = @json(array_values($temperatureDistribution));
    tempOpts.labels = @json(array_keys($temperatureDistribution));
    ...
    respOpts.series = @json(array_values($respiratoryRateDistribution));
    respOpts.labels = @json(array_keys($respiratoryRateDistribution));
