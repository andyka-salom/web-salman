...
<div class="kanban-card priority-{{ $priority }}"
     ...
     data-due-date="{{ $item->target_date->toDateString() }}">

    <div class="card-header-section">
        <span class="priority-badge bg-light-{{ strtolower($priorityColor) }}">
            {{ $priority }} Priority
        </span>
    ...
    <div class="card-meta">
        <span class="meta-badge">
            ...
            <a href="{{ route('cermat.reports.show', $item->cermat_report_id) }}"
               target="_blank"
        ...
        <span class="meta-badge meta-area">
            ...
            <span title="{{ optional($item->cermatReport->area)->name ?? 'N/A' }}">
                {{ Str::limit(optional($item->cermatReport->area)->name ?? 'N/A', 20) }}
            </span>
        </span>
        ...
        @if($item->actionCategory)
            <span class="meta-badge meta-category">
                <i class="bi bi-tag"></i>
                {{ Str::limit($item->actionCategory->name, 25) }}
            </span>
        @endif
    </div>

    <div class="card-footer">
        <div class="date-meta {{ $dueDateStatus }}" title="{{ $item->target_date->isoFormat('dddd, DD MMMM YYYY') }}">
            <i class="bi {{ $isOverdue ? 'bi-calendar-x' : 'bi-calendar3' }}"></i>
            <div class="date-info">
                <span class="date-value">{{ $item->target_date->isoFormat('DD MMM YYYY') }}</span>
                <span class="date-relative">{{ $dueDateText }}</span>
            </div>
        </div>
        ...
        <div class="card-indicators">
            ...
            @if($item->photos->isNotEmpty())
                <div class="indicator indicator-photos" title="{{ $item->photos->count() }} photo(s)">
                    <i class="bi bi-images"></i>
                    <span class="indicator-count">{{ $item->photos->count() }}</span>
                </div>
            @endif
        </div>
    </div>
