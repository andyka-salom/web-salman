{{-- resources/views/cermat/form.blade.php --}}
        ...
        <div class="row layout-top-spacing">
            <div class="col-12">
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                    <div>
                        <h3 class="fw-bold mb-0 text-dark">{{ $pageTitle }}</h3>
                ...
                <form action="{{ $actionRoute }}" method="POST" id="cermat-form" enctype="multipart/form-data">
                    @csrf
                    @if($isEdit) @method('PUT') @endif

                    {{-- Hidden Field untuk Supervisor ID (Auto-generated) --}}
                    <input type="hidden" name="line_supervisor_id" value="{{ $supervisorId }}">

                    {{-- Field Requires Feedback Hidden dengan Default Value --}}
                    @php
                        $feedbackValue = $getData('requires_feedback', '1'); // Default: Ya (1)
                    @endphp
                    <input type="hidden" name="requires_feedback" value="{{ $feedbackValue }}">

                    {{-- Data Persistence untuk Field Disabled (Requirement Laravel Validation) --}}
                    @if($isHsseReview)
                        <input type="hidden" name="details" value="{{ $getData('details') }}">
                        <input type="hidden" name="report_datetime" value="{{ $getData('report_datetime') }}">
                        <input type="hidden" name="area_id" value="{{ $getData('area_id') }}">
                        <input type="hidden" name="location_details" value="{{ $getData('location_details') }}">
                        <input type="hidden" name="immediate_action_taken" value="{{ $getData('immediate_action_taken') }}">
                        <input type="hidden" name="stop_card_issued" value="{{ $getData('stop_card_issued') }}">
                        ...
                        <div class="col-lg-8">
                            <div class="d-flex flex-column gap-4">
                                ...
                                <div class="card shadow-sm border-0 br-8">
                                    <div class="card-body p-4">
                                        <div class="form-section-header">
                                            <div class="step-circle" @if($isHsseReview) style="background:#eee;color:#999" @endif>1</div>
                                            <h5 class="mb-0 fw-bold">Detail Kejadian</h5>
                                            @if($isHsseReview) <span class="badge bg-light-dark text-dark ms-auto">Read Only</span> @endif
                                        </div>

                                        <div class="mb-4">
                                            <label class="form-label fw-bold">Uraian / Kronologi <span class="text-danger">*</span></label>
                                            <textarea name="details" class="form-control @error('details') is-invalid @enderror" rows="5" {{ $disabledAttr }} placeholder="Jelaskan secara detail temuan Anda...">{{ $getData('details') }}</textarea>
                                        ...
                                        <div class="row g-3 mb-4">
                                            <div class="col-md-6">
                                                ...
                                                @endphp
                                                <input type="datetime-local" name="report_datetime" id="report_datetime" class="form-control @error('report_datetime') is-invalid @enderror" value="{{ $dtDisplay }}" {{ $disabledAttr }} max="{{ now()->format('Y-m-d\TH:i') }}">
                                                <small class="text-muted">* Minimal 1 menit sebelum waktu sekarang</small>
                                                <div class="invalid-feedback">@error('report_datetime') {{ $message }} @enderror</div>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label fw-bold">Area Lokasi <span class="text-danger">*</span></label>
                                                <select id="area_id" name="area_id" class="form-select @error('area_id') is-invalid @enderror" {{ $disabledAttr }}></select>
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="location_details" class="form-control @error('location_details') is-invalid @enderror" value="{{ $getData('location_details') }}" {{ $disabledAttr }} placeholder="Misal: Deck 2, Engine Room, dsb.">
                                            <div class="invalid-feedback">@error('location_details') {{ $message }} @enderror</div>
                                        </div>
                                    </div>
                                </div>

                                {{-- SECTION 2: KLASIFIKASI (SELALU EDITABLE BAGI HSSE) --}}
                                <div class="card shadow-sm border-0 border-start border-primary border-4 br-8">
                                    <div class="card-body p-4">
                                        <div class="form-section-header">
                                            ...
                                            @if($isHsseReview) <span class="badge bg-light-success text-success ms-auto">HSSE Edit Mode</span> @endif
                                        </div>
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                ...
                                <div class="card shadow-sm border-0 br-8">
                                    <div class="card-body p-4">
                                        <div class="form-section-header">
                                            <div class="step-circle" @if($isHsseReview) style="background:#eee;color:#999" @endif>3</div>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <textarea name="immediate_action_taken" class="form-control @error('immediate_action_taken') is-invalid @enderror" rows="3" {{ $disabledAttr }} placeholder="Tindakan apa yang langsung dilakukan?">{{ $getData('immediate_action_taken') }}</textarea>
                                            <div class="invalid-feedback">@error('immediate_action_taken') {{ $message }} @enderror</div>
                                        </div>

                                        {{-- INFO SUPERVISOR (READ-ONLY) --}}
                                        <div class="mb-4">
                                            ...
                                            <div class="supervisor-info-box">
                                                <div class="d-flex align-items-center gap-3">
                                                    <div class="icon-box">
                                                        <i class="bi {{ $supervisor ? 'bi-person-badge-fill' : 'bi-building' }} fs-4"></i>
                                                    </div>
                                                    <div class="flex-grow-1">
                                                        <h6 class="mb-1 fw-bold">{{ $supervisorName }}</h6>
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-12">
                                                ...
                                                @php $stopVal = $getData('stop_card_issued'); @endphp
                                                <div class="btn-group w-100">
                                                    <input type="radio" class="btn-check" name="stop_card_issued" id="stop_yes" value="1" {{ $stopVal == '1' ? 'checked' : '' }} {{ $disabledAttr }}>
                                                    <label class="btn btn-outline-primary" for="stop_yes">Ya</label>
                                                    <input type="radio" class="btn-check" name="stop_card_issued" id="stop_no" value="0" {{ ($stopVal === '0' || $stopVal === 0) ? 'checked' : '' }} {{ $disabledAttr }}>
                                ...
                                <div class="card shadow-sm border-0 br-8">
                                    <div class="card-body p-4">
                                        <div class="form-section-header">
                                            <div class="step-circle" @if($isHsseReview) style="background:#eee;color:#999" @endif>4</div>
                                            <h5 class="mb-0 fw-bold">Bukti Foto Dokumentasi</h5>
                                        </div>

                                        @if($isEdit && $report->attachments->isNotEmpty())
                                            <div class="mb-4">
                                                ...
                                                @if(!$isHsseReview) <small class="text-danger">* Centang foto jika ingin menghapus.</small> @endif
                                            </div>
                                        @endif
                                        ...
                                        <div id="upload-area">
                                            ...
                                            {{-- Button Group untuk Kamera dan Upload --}}
                                    ...
                                    <div class="card-body p-4 text-center">
                                        ...
                                        <div class="d-grid gap-2">
                                            <button type="submit" class="btn btn-primary fw-bold py-3 shadow-sm" id="submit-btn">
                                                <i class="bi bi-send-fill me-2"></i> {{ $submitText }}
                                            </button>
                                            <a href="{{ $cancelRoute }}" class="btn btn-outline-secondary py-2">Batal</a>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card shadow-sm border-0 br-8 mt-3 bg-light-info border-0">
                                    <div class="card-body p-4">
                                        ...
                                        <ul class="small text-muted ps-3 mb-0">
                                            @if($supervisor)
                                            <li>Laporan akan ditinjau oleh {{ $supervisor->name }}.</li>
                                            @else
...
document.addEventListener('DOMContentLoaded', function() {
    const isHsseReview = @json($isHsseReview);
    const maxFiles = 5;
    ...
    new TomSelect('#area_id', { ...tsBase, placeholder: 'Pilih Area...', options: @json($areas->map(fn($i)=>['value'=>$i->id,'text'=>$i->name])), items: [@json($getData('area_id'))].filter(Boolean) });
    ...
    new TomSelect('#unsafe_acts', { ...tsMulti, options: @json($unsafeActs->map(fn($i)=>['value'=>$i->id,'text'=>$i->description])), items: @json($getArrayData('unsafeActs', 'selectedUnsafeActs')) });
    new TomSelect('#unsafe_conditions', { ...tsMulti, options: @json($unsafeConditions->map(fn($i)=>['value'=>$i->id,'text'=>$i->description])), items: @json($getArrayData('unsafeConditions', 'selectedUnsafeConditions')) });
    ...
    $('#cermat-form').on('submit', function(e) {
        ...
        const existingCount = @json($isEdit ? $report->attachments->count() : 0);
