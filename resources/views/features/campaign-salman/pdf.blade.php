...
<head>
    ...
    <title>Laporan Kegiatan - {{ $campaignSalman->tema }}</title>
    <style>
        @page {
            margin: 0px;
            size: A4;
...
<body>
    ...
    <div class="cover-page">
        @if(!isset($campaignSalman->coverTemplate) || !$campaignSalman->coverTemplate)
            <!-- Fallback Cover tanpa Template -->
            <div class="cover-fallback">
                <div class="cover-content-fallback">
                    <div class="cover-title">{{ $campaignSalman->tema }}</div>

                    <!-- NAMA PERUSAHAAN -->
                    @if($campaignSalman->company)
                        <div class="cover-company">{{ $campaignSalman->company->name }}</div>
                    @endif

                    <p style="font-size: 20px; margin: 0;">{{ $campaignSalman->entitas }}</p>
                    <br>
                    <p style="font-size: 16px; opacity: 0.8;">
                        {{ $campaignSalman->tanggal->isoFormat('D MMMM Y') }}
                    </p>
                </div>
            </div>
        @else
            ...
            <div class="cover-overlay">
                <div class="cover-overlay-title">{{ $campaignSalman->tema }}</div>

                <!-- NAMA PERUSAHAAN -->
                @if($campaignSalman->company)
                    <div class="cover-overlay-company">{{ $campaignSalman->company->name }}</div>
                @endif

                <div class="cover-overlay-entitas">{{ $campaignSalman->entitas }}</div>
                <div class="cover-overlay-date">{{ $campaignSalman->tanggal->isoFormat('dddd, D MMMM Y') }}</div>
            </div>
        @endif
    </div>
    ...
    <table class="details-table">
        ...
        <tr>
            ...
            <td class="details-value">{{ $campaignSalman->company->name }}</td>
        </tr>
        @endif

        <tr>
            <td class="details-label">Tema Kegiatan</td>
            <td class="details-value">{{ $campaignSalman->tema }}</td>
        </tr>
        <tr>
            <td class="details-label">Tanggal Pelaksanaan</td>
            <td class="details-value">{{ $campaignSalman->tanggal->isoFormat('dddd, D MMMM Y') }}</td>
        </tr>
        <tr>
            <td class="details-label">Lokasi</td>
            <td class="details-value">{{ $campaignSalman->lokasi }}</td>
        </tr>
        <tr>
            <td class="details-label">Jumlah Peserta</td>
            <td class="details-value">{{ $campaignSalman->jumlah_peserta }} Orang</td>
        </tr>
        <tr>
            <td class="details-label">Pemateri</td>
            <td class="details-value">{{ $campaignSalman->pemateri }}</td>
        </tr>
        <tr>
            <td class="details-label">Entitas</td>
            <td class="details-value">{{ $campaignSalman->entitas }}</td>
        </tr>
    </table>
    ...
    <div class="content-text">
        {!! nl2br(e($campaignSalman->ringkasan)) !!}
    </div>

    <!-- Dokumentasi -->
    @if(isset($campaignSalman->dokumentasi) && count($campaignSalman->dokumentasi) > 0)
    ...
    <!-- Daftar Hadir -->
    @if(isset($campaignSalman->daftar_hadir) && count($campaignSalman->daftar_hadir) > 0)
