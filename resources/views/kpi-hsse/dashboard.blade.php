{{-- resources/views/kpi-hsse/dashboard.blade.php — Production v4 --}}
@extends('layouts.app')
@section('title', 'Dashboard KPI HSSE')
...
@section('content')
    ...
    <div class="col-12">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div>
                ...
                <p class="text-muted mb-0" style="font-size:.84rem;">
                    @if($isHsse) Semua Perusahaan Kontraktor
                    @else <strong>{{ auth()->user()->company->name ?? '' }}</strong>
                    @endif
                </p>
            </div>
            <a href="{{ route('kpi-hsse.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill px-3">
                📋 Daftar Laporan
            </a>
        </div>
        ...
        <form method="GET" class="row g-2 mb-4 align-items-end">
            ...
            <div class="col-auto">
                ...
                <select name="period_year" class="form-select form-select-sm">
                    ...
                    @foreach($periods->pluck('period_year')->unique()->sortDesc() as $yr)
                    ...
                    <div class="dash-panel-body">
                        ...
                        @foreach($pyrItems as $idx => $pyr)
                            ...
                            <div class="pyramid-row">
                                <div class="pyr-label">{{ $sn }}</div>
                                <div class="pyr-right">
                                    <div class="pyr-bar" style="width:{{ $pct }}%;background:{{ $color }};">
                                        <span>{{ number_format($pyr['total_actual']) }}</span>
                                        @if(!$isAR && $pyr['avg_score'] > 0)
                                            <span class="pyr-score">sc:{{ number_format($pyr['avg_score'],1) }}</span>
                                        @else
                        ...
                        @php $sumAll = collect($laggingPyramidData)->sum('total_actual'); @endphp
                        <div class="pyramid-note">
                            <strong>∑ Total Lagging:</strong> {{ number_format($sumAll) }} implementasi
                        </div>
        ...
        <div class="row g-4 mb-4">
            ...
            @if($isHsse && $topCompanies->count() > 0)
                <div class="col-lg-4">
                    <div class="dash-panel h-100">
                        <div class="dash-panel-hdr">
                        ...
                        <div class="dash-panel-body">
                            @foreach($topCompanies->take(8) as $i => $co)
                                @php
                                ...
                                <div class="top-row">
                                    ...
                                    <div style="flex:1;min-width:0;">
                                        <div style="font-size:.78rem;font-weight:700;color:#1e293b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $co->name }}</div>
                                        <div class="top-bar-wrap mt-1">
                                            <div class="top-bar-fill" style="width:{{ $barPct }}%;background:{{ $barCol }};"></div>
                                        </div>
                                    </div>
                                    <div style="font-size:.82rem;font-weight:700;color:{{ $barCol }};min-width:36px;text-align:right;">{{ $avgSc }}</div>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <table class="table table-sm table-hover mb-0 recent-tbl">
                        <thead class="table-light">
                            <tr>
                                ...
                                @if($isHsse)<th style="font-size:.78rem;padding:.6rem .75rem;">Perusahaan</th>@endif
                        ...
                        <tbody id="tbl-body">
                        @foreach($recentReports as $idx => $r)
                            @php
                                $sc = $r->total_score;
                                $cl = $sc ? ($sc>=90?'excellent':($sc>=75?'good':($sc>=60?'fair':'poor'))) : 'none';
                            @endphp
                            <tr class="tbl-row"
                                data-search="{{ strtolower(($r->company->name ?? '') . ' ' . ($r->kpiPeriod->label ?? '') . ' ' . $r->status) }}">
                                <td style="color:#9ca3af;font-size:.75rem;vertical-align:middle;">{{ $idx+1 }}</td>
                                @if($isHsse)
                                    <td style="vertical-align:middle;"><strong style="font-size:.82rem;">{{ $r->company->name ?? '-' }}</strong></td>
                                @endif
                                <td style="font-size:.82rem;vertical-align:middle;">{{ $r->kpiPeriod->label ?? '-' }}</td>
                                <td style="vertical-align:middle;">
                                    <span class="badge-kpi-status badge-status-{{ $r->status }}">{{ ucfirst($r->status) }}</span>
                                </td>
                                <td style="text-align:center;vertical-align:middle;">
                                    @if($sc)
                                        <span class="score-pill {{ $cl }}" style="width:38px;height:38px;font-size:.72rem;">{{ number_format($sc,0) }}</span>
                                    @else
                                        <span class="score-pill none" style="width:38px;height:38px;font-size:.72rem;">—</span>
                                    @endif
                                </td>
                                <td style="font-size:.76rem;color:#9ca3af;vertical-align:middle;">{{ $r->updated_at->diffForHumans() }}</td>
                                <td style="vertical-align:middle;">
...
@endphp
const chartCompanies = @json($chartCompanies);
