{{--
resources/views/kpi-hsse/form.blade.php — Production v13
═══════════════════════════════════════════════════════════════════════════════
Modes : create | edit | show
Roles : koordinator | hsse | super-admin
Steps : 0 Info & Kapal → 1 Lagging → 2 Leading

Fix v13:
- Lagging item No.6 & 7 (As Reported, FAC & Nearmiss) sekarang WAJIB
  direview oleh HSSE (Accept / Reject) meskipun tidak ada score/nilai.
- Item No.8 (Manhours, As Reported) tetap tidak perlu review.
- Leading As Reported tetap tidak perlu review.
- Remark HSSE ditampilkan sebagai alert box merah yang menonjol
  di sisi koordinator (bukan hanya teks kecil italic).
- Progress review sidebar ikut menghitung item 6-7.
--}}
@extends('layouts.app')
...
<div class="kpi-header">
    <div class="kpi-header-left">
        <h1 class="kpi-header-title">
            @if($isCreate) Buat Laporan KPI HSSE
            @elseif($isEdit) Edit KPI HSSE
            @else Detail KPI HSSE
            @endif
        </h1>
        <div class="kpi-header-meta">
            @if($report)
                ...
                <span class="sep">•</span>
                <span>{{ $report->kpiPeriod->label ?? '-' }}</span>
                <span class="sep">•</span>
                <span class="badge badge-{{ $rStatus }}">
                    {{ ['draft'=>'Draft','submitted'=>'Submitted','validated'=>'Validated','rejected'=>'Rejected'][$rStatus] ?? ucfirst($rStatus) }}
                </span>
            @else
...
@foreach(['success','error'] as $k)
    @if(session($k))
        <div class="alert alert-{{ $k==='success'?'success':'danger' }} alert-dismissible fade show mb-3"
...
<div class="wizard-wrap">
    <div class="wizard-track">
        <div class="wizard-step active" id="ws-0" onclick="{{ !$isCreate ? 'jumpStep(0)' : '' }}">
        ...
        <div class="wizard-step" id="ws-1" onclick="jumpStep(1)">
            <div class="ws-circle">
                ...
                @if($rejCnt > 0)
                <span style="position:absolute;top:-4px;right:-4px;width:14px;height:14px;border-radius:50%;background:#ef4444;color:#fff;font-size:8px;display:flex;align-items:center;justify-content:center;font-weight:800;border:2px solid #fff;">{{ $rejCnt }}</span>
                @endif
            </div>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div>
                ...
                <p class="card-sub">{{ $isCreate ? 'Pilih bulan dan tahun laporan KPI' : 'Periode laporan ini' }}</p>
            </div>
        </div>
        <div class="card-body">
            @if($isCreate)
            <div class="field">
                ...
                <select name="period" class="field-select" style="max-width:280px;" required>
                    ...
                    @forelse($availableMonths as $mo)
                        <option value="{{ $mo['value'] }}" {{ old('period')==$mo['value']?'selected':'' }}>{{ $mo['label'] }}</option>
                    @empty
                        <option disabled>Semua periode sudah ada laporan</option>
                    @endforelse
                </select>
            </div>
            @elseif($isEdit && ($isKoord || $isSA))
            <div class="field mb-0">
                ...
                <div style="display:flex;align-items:center;gap:.75rem;">
                    <select id="select-period-edit" class="field-select" style="max-width:280px;" 
                        onchange="doUpdatePeriod(this.value, '{{ route('kpi-hsse.period.update_report', $report) }}')">
                        @php 
                            $currKey = $report->kpiPeriod->period_month . '-' . $report->kpiPeriod->period_year;
                            $hasCurr = false;
                        @endphp
                        @foreach($availableMonths as $mo)
                            @if($mo['value'] == $currKey) @php $hasCurr = true; @endphp @endif
                            <option value="{{ $mo['value'] }}" {{ $mo['value'] == $currKey ? 'selected' : '' }}>
                                {{ $mo['label'] }}
                            </option>
                        @endforeach
                        @if(!$hasCurr)
                            <option value="{{ $currKey }}" selected>{{ $report->kpiPeriod->label }}</option>
                        @endif
                    </select>
            ...
            <div style="display:inline-flex;align-items:center;gap:.75rem;background:var(--blue-light);border:1px solid var(--blue-mid);border-radius:var(--radius-sm);padding:.65rem 1.1rem;">
                ...
                <div>
                    <div id="label-period-display" style="font-weight:700;color:#1e40af;font-size:.9rem;">{{ $report->kpiPeriod->label ?? '-' }}</div>
                    @if($report->kpiPeriod?->submission_end)
                    <div style="font-size:.72rem;color:#3b82f6;margin-top:2px;">Batas submit: {{ $report->kpiPeriod->submission_end->format('d M Y') }}</div>
                    @endif
                </div>
            </div>
            @endif
            ...
            <div class="vtbl-wrap">
                ...
                <table class="vtbl" id="vessel-table">
                    ...
                    <tbody id="vessel-tbody">
                        <tr id="vrow-0">
                            <td class="td-no">1</td>
                            <td><input type="text" class="vtbl-inp" name="vessels[0][contract_number]" value="{{ old('vessels.0.contract_number') }}" placeholder="PHM/2025/001"></td>
                            <td><input type="date" class="vtbl-inp" name="vessels[0][contract_end_date]" value="{{ old('vessels.0.contract_end_date') }}"></td>
                            <td><input type="text" class="vtbl-inp" name="vessels[0][vessel_count]" value="{{ old('vessels.0.vessel_count') }}" placeholder="1 unit"></td>
                            <td><input type="text" class="vtbl-inp" name="vessels[0][vessel_name]" value="{{ old('vessels.0.vessel_name') }}" placeholder="KM Harapan Jaya" required></td>
                ...
                <table class="vtbl">
                    <thead><tr>
                        ...
                        @if($canEditKoord)<th class="td-del"></th>@endif
                    </tr></thead>
                    <tbody id="vessel-tbody">
                        @foreach($report->vessels->sortBy('sort_order') as $vi => $v)
                        <tr id="vrow-{{ $v->id }}">
                            <td class="td-no">{{ $vi+1 }}</td>
                            @if($canEditKoord)
                            <td><input type="text" class="vtbl-inp" id="vc-{{ $v->id }}" value="{{ $v->contract_number }}" placeholder="No. kontrak" onblur="doSaveVessel('{{ $v->id }}','{{ route('kpi-hsse.vessels.update',[$report,$v]) }}')"></td>
                            <td><input type="date" class="vtbl-inp" id="vd-{{ $v->id }}" value="{{ $v->contract_end_date?->format('Y-m-d') }}" onblur="doSaveVessel('{{ $v->id }}','{{ route('kpi-hsse.vessels.update',[$report,$v]) }}')"></td>
                            <td><input type="text" class="vtbl-inp" id="vco-{{ $v->id }}" value="{{ $v->vessel_count }}" placeholder="1 unit" onblur="doSaveVessel('{{ $v->id }}','{{ route('kpi-hsse.vessels.update',[$report,$v]) }}')"></td>
                            <td><input type="text" class="vtbl-inp" id="vn-{{ $v->id }}" value="{{ $v->vessel_name }}" placeholder="Nama kapal" onblur="doSaveVessel('{{ $v->id }}','{{ route('kpi-hsse.vessels.update',[$report,$v]) }}')"></td>
                            <td class="td-del">
                                <button type="button" class="btn-del-vessel {{ $report->vessels->count()<=1?'d-none':'' }}" onclick="doDelVessel('{{ $v->id }}','{{ route('kpi-hsse.vessels.destroy',[$report,$v]) }}')">×</button>
                            </td>
                            @else
                            <td style="padding:.5rem .75rem;font-size:.82rem;">{{ $v->contract_number ?: '—' }}</td>
                            <td style="padding:.5rem .75rem;font-size:.82rem;">{{ $v->contract_end_date?->format('d/m/Y') ?? '—' }}</td>
                            <td style="padding:.5rem .75rem;font-size:.82rem;">{{ $v->vessel_count ?: '—' }}</td>
                            <td style="padding:.5rem .75rem;font-size:.82rem;font-weight:600;">{{ $v->vessel_name }}</td>
                            @endif
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="card-body">
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(175px,1fr));gap:1rem;">
                <div>
                    ...
                    <div style="font-weight:600;font-size:.85rem;">{{ $report->createdBy->name ?? '-' }}</div>
                    <div style="font-size:.72rem;color:var(--gray-400);">{{ $report->created_at->format('d M Y, H:i') }}</div>
                </div>
                @if($report->submitted_at)
                <div>
                    <div class="field-lbl">Disubmit</div>
                    <div style="font-weight:600;font-size:.85rem;">{{ $report->submittedBy->name ?? '-' }}</div>
                    <div style="font-size:.72rem;color:var(--gray-400);">{{ $report->submitted_at->format('d M Y, H:i') }}</div>
                </div>
                @endif
                @if($report->validated_at)
                <div>
                    <div class="field-lbl">Divalidasi</div>
                    <div style="font-weight:600;font-size:.85rem;">{{ $report->validatedBy->name ?? '-' }}</div>
                    <div style="font-size:.72rem;color:var(--gray-400);">{{ $report->validated_at->format('d M Y, H:i') }}</div>
                </div>
                @endif
                <div>
                    <div class="field-lbl">Update terakhir</div>
                    <div style="font-size:.82rem;color:var(--gray-400);">{{ $report->updated_at->diffForHumans() }}</div>
                </div>
            </div>
        </div>
    </div>
...
<div class="step-panel" id="step-1">
    ...
    <div class="card" style="padding:0;overflow:hidden;">
        ...
        <table class="kpi-tbl">
            ...
            @php
                ...
                $upId       = $det?->id ?? '';
            @endphp
            <tr class="kpi-row {{ $rowCls }}" data-section="lagging" data-item-no="{{ $item->item_no }}">
                <td class="td-no-kpi">{{ $item->item_no }}</td>

                {{-- ── ITEM NAME ── --}}
                <td class="td-item">
                    <div class="item-name">{{ $iName }}</div>
                    @if($iTarget)<div class="item-target">🎯 {{ $iTarget }}</div>@endif
                    <div class="item-tags">
                        @if($isAS && $needsReview)
                        ...
                        @endif
                        @if($isRej)<span class="itag itag-rej">✗ Ditolak</span>@endif
                        @if($isOK) <span class="itag itag-ok">✓ Disetujui</span>@endif
                    </div>
                    {{-- REMARK BOX — menonjol untuk koordinator --}}
                    @if($isRej && $det?->hsse_catatan)
                    <div class="rej-remark-box">
                        <div class="rej-remark-label">💬 Catatan HSSE</div>
                        <div class="rej-remark-text">{{ $det->hsse_catatan }}</div>
                    </div>
                    @endif
                </td>
                ...
                <td class="td-jml">
                    ...
                    <div class="jml-wrap">
                        <input type="number"
                            ...
                            placeholder="0"
                            data-item="{{ $item->id }}"
                            data-save-url="{{ route('kpi-hsse.update', $report) }}"
                    ...
                    <div class="jml-readonly">
                        {{ $det?->actual_count ?? '—' }}
                        <div class="jml-unit">{{ $unitLabel }}</div>
                    </div>
                    @endif
                </td>
                ...
                <td class="td-ket">
                    @if($canEditKoord)
                    <textarea class="ket-inp" id="ket-{{ $item->id }}" rows="2"
                        placeholder="Keterangan singkat..."
                        data-item="{{ $item->id }}"
                        data-save-url="{{ route('kpi-hsse.update', $report) }}"
                        onblur="saveKet(this)">{{ old('items.'.$item->id.'.keterangan', $det?->keterangan) }}</textarea>
                    @else
                    <div class="ket-readonly">{{ $det?->keterangan ?: '—' }}</div>
                    @endif
                </td>
                ...
                <td class="td-ev">
                    @if($isShared)
                        ...
                        @if($isAnchor)
                            {{-- ANCHOR No.1: upload button + grid --}}
                            @if($canEditKoord)
                            ...
                            <input type="file" id="ev-up-anchor-lag" class="d-none"
                                ...
                                data-anchor-detail-id="{{ $lagAnchorId }}"
                                data-anchor-item-id="{{ $lagAnchorItem?->id ?? '' }}"
                                data-report-id="{{ $report->id }}"
                                data-url="{{ route('kpi-hsse.evidences.upload', $report) }}">
                            ...
                            <div class="ev-grid" id="ev-row-lag-1">
                                ...
                                <div class="ev-item" id="ev-item-lag1-{{ $ev->id }}">
                                    ...
                                    @endif
                                    @if($canEditKoord)
                                    <div class="ev-del-btn" onclick="doDelEvShared('{{ $ev->id }}','{{ route('kpi-hsse.evidences.delete',[$report,$ev]) }}','{{ $lagAnchorId }}')">×</div>
                                    @endif
                                </div>
                                @endforeach
                            </div>
                            <div id="ev-st-lag-1" style="margin-top:3px;">
                                @if($lagAnchorEvCnt > 0)
                                    <span class="ev-stat-ok">{{ $lagAnchorEvCnt }} file</span>
                                @elseif($canEditKoord)
                                    <span class="ev-stat-req">Wajib</span>
                                @endif
                            </div>
                        @else
                            {{-- No.2–7: hint + thumbnails read-only --}}
                            @if($canEditKoord)
                            <span class="ev-shared-sub">↑ Upload di baris No.1</span>
                            @endif
                            <div class="ev-grid" id="ev-row-lag-{{ $item->item_no }}">
                                @foreach($lagAnchorEvs as $ev)
                                @php $isPdf = str_contains($ev->file_type ?? '', 'pdf'); @endphp
                            ...
                            <div id="ev-st-lag-{{ $item->item_no }}" style="margin-top:3px;">
                                @if($lagAnchorEvCnt > 0)
                                    <span class="ev-stat-ok">{{ $lagAnchorEvCnt }} file</span>
                                @elseif($canEditKoord)
                                    <span class="ev-stat-req">Wajib</span>
                    ...
                    @else
                        ...
                        <div class="ev-grid" id="ev-row-{{ $upId }}">
                            @if($det)
                            ...
                            <div class="ev-item" id="ev-item-{{ $ev->id }}">
                                ...
                                @endif
                                @if($canEditKoord)
                                <div class="ev-del-btn" onclick="doDelEv('{{ $ev->id }}','{{ route('kpi-hsse.evidences.delete',[$report,$ev]) }}','{{ $upId }}')">×</div>
                                @endif
                            </div>
                            @endforeach
                            @endif
                        </div>
                        @if($canEditKoord && $upId)
                        <label class="ev-upload-btn" id="lbl-{{ $upId }}" for="ev-up-{{ $upId }}">+ Lampiran</label>
                        ...
                        @elseif($canEditKoord && !$upId)
                        <span class="ev-hint">Isi ∑/% dulu</span>
                        @endif
                        @php $evCntNs = $det ? $det->evidences->count() : 0; @endphp
                        <div id="ev-st-{{ $upId }}" style="margin-top:4px;">
                            @if($evCntNs > 0)
                                <span class="ev-stat-ok">{{ $evCntNs }} file</span>
                            @elseif($canEditKoord)
                                <span class="ev-stat-req">Wajib</span>
                            @endif
                        </div>
                ...
                {{-- ── NILAI / BOBOT / SCORE / REVIEW (HSSE columns) ── --}}
                @if($isHsse || $isSA)
                <td class="td-nilai">
                    @if($isAS)
                        <span class="nilai-ar">A/R</span>
                    @else
                        <input type="number" class="nilai-inp" id="nilai-{{ $item->id }}"
                            value="{{ $nilaiVal > 0 ? number_format($nilaiVal,2,'.','') : '' }}"
                ...
                <td class="td-bobot">
                    @if($item->is_scored)
                        {{ number_format((float)$item->bobot*100,1) }}%
                    @else
                        <span class="td-bobot-na">—</span>
                    @endif
                </td>
                <td class="td-score {{ $scClsRow }}" id="sc-{{ $item->id }}">
                    {{ $isAS ? 'A/R' : ($score !== null ? number_format((float)$score,2) : '—') }}
                </td>

                {{-- ── REVIEW COLUMN ── --}}
                <td class="td-review">
                    @if($isArNoReview)
                        {{-- Item No.8 Manhours: tidak perlu review --}}
                        <div class="inline-review ar-zone">
                            <span class="ir-label ar">As Reported — tidak direview</span>
                        </div>
                    @elseif($canDoInlineReview)
                        {{-- Item scored (1-5) dan As Reported reviewable (6-7): tampilkan tombol Accept/Reject --}}
                        <div class="inline-review{{ ($isAS && $needsReview) ? ' ar-review-zone' : '' }}">
                            @if($isAS && $needsReview)
                                <span class="ir-label ar-review">Verifikasi Laporan (A/R)</span>
                            @else
                                <span class="ir-label">Catatan Verifikasi</span>
                            @endif
                            <textarea class="ir-catatan" id="ircmt-{{ $dId }}" rows="2"
                                placeholder="Catatan untuk koordinator..."
                                data-detail="{{ $dId }}"
                                data-url="{{ route('kpi-hsse.catatan.update', $report) }}"
                                onblur="saveCat(this)">{{ $det?->hsse_catatan }}</textarea>
                            <div class="ir-dec-row" id="irdec-{{ $dId }}">
                                <button type="button"
                                    class="ir-dec-btn ir-ok {{ $isOK ? 'active' : '' }}"
                                    id="irbtn-ap-{{ $dId }}"
                                    onclick="doInlineDecision('{{ $dId }}','approved','{{ route('kpi-hsse.decision.update',$report) }}','{{ $item->id }}')">
                                    ✓ Accept
                                </button>
                                <button type="button"
                                    class="ir-dec-btn ir-rej {{ $isRej ? 'active' : '' }}"
                                    id="irbtn-rj-{{ $dId }}"
                                    onclick="doInlineDecision('{{ $dId }}','rejected','{{ route('kpi-hsse.decision.update',$report) }}','{{ $item->id }}')">
                                    ✗ Reject
                                </button>
                            </div>
                        </div>
                    @else
                        {{-- Show-mode: tampilkan status review saja --}}
                        <div class="inline-review" style="background:var(--gray-50);border-left-color:var(--gray-200);">
                            ...
                            @elseif($isRej)
                                ...
                                @if($det?->hsse_catatan)
                                <div style="font-size:.67rem;color:#b91c1c;margin-top:4px;font-style:italic;">{{ $det->hsse_catatan }}</div>
                                @endif
                            @else
    ...
    <div class="step-nav">
        ...
        <div style="display:flex;gap:.6rem;align-items:center;">
            @if(!$isCreate && ($canEditKoord || $canEditHsse || $isSA))
...
<div class="step-panel" id="step-2">
    ...
    <div class="card" style="padding:0;overflow:hidden;">
        ...
        <table class="kpi-tbl">
            ...
            <tbody>
            @php $leadItems = $kpiItems->get('leading', collect()); @endphp
            ...
            @php
                ...
                $dId        = $det?->id ?? '';
            @endphp
            <tr class="kpi-row {{ $rowCls }}" data-section="leading" data-item-no="{{ $item->item_no }}">
                <td class="td-no-kpi">{{ $item->item_no }}</td>
                ...
                <td class="td-item">
                    ...
                    @if($iTarget)<div class="item-target">🎯 {{ $iTarget }}</div>@endif
                    <div class="item-tags">
                        @if($isAS)<span class="itag itag-ar">As Reported</span>@endif
                        @if($isRej)<span class="itag itag-rej">✗ Ditolak</span>@endif
                        @if($isOK) <span class="itag itag-ok">✓ Disetujui</span>@endif
                    </div>
                    {{-- REMARK BOX menonjol untuk koordinator --}}
                ...
                <td class="td-jml">
                    @if($canEditKoord)
                    <div class="jml-wrap">
                        <input type="number" class="jml-inp" id="ac-{{ $item->id }}"
                            value="{{ old('items.'.$item->id.'.actual_count', $det?->actual_count) }}"
                            min="0" step="{{ $isPercent ? '0.01' : '1' }}"
                            placeholder="{{ $isAS ? '—' : '0' }}"
                            {{ $isAS ? 'disabled' : '' }}
                ...
                <td class="td-ev">
                    ...
                    @else
                        ...
                        <div class="ev-prog" id="ev-prog-{{ $upId }}"><div class="ev-bar" id="ev-bar-{{ $upId }}"></div></div>
                        <div class="ev-err" id="ev-err-{{ $upId }}"></div>
                        @elseif($canEditKoord && !$det)
                        <span class="ev-hint">Isi ∑/% dulu</span>
                        @endif
                        <div id="ev-st-{{ $upId }}" style="margin-top:4px;">
                            @if($evCnt > 0)
                                <span class="ev-stat-ok">{{ $evCnt }} file</span>
                ...
                {{-- ── NILAI / BOBOT / SCORE / REVIEW ── --}}
                @if($isHsse || $isSA)
                <td class="td-nilai">
                    @if($isAS)<span class="nilai-ar">A/R</span>
                    @else
                    <input type="number" class="nilai-inp" id="nilai-{{ $item->id }}"
                        value="{{ $nilaiVal > 0 ? number_format($nilaiVal,2,'.','') : '' }}"
                        min="0" max="100" step="0.01"
                ...
                <td class="td-bobot">
                    ...
                    @else <span class="td-bobot-na">—</span>@endif
                </td>
                <td class="td-score {{ $scClsRow }}" id="sc-{{ $item->id }}">
                    {{ $isAS ? 'A/R' : ($score !== null ? number_format((float)$score,2) : '—') }}
                </td>

                {{-- Leading: As Reported → tidak direview --}}
                <td class="td-review">
                    @if($isAS)
                        <div class="inline-review ar-zone">
                            <span class="ir-label ar">As Reported — tidak direview</span>
                    ...
                    @else
                        <div class="inline-review" style="background:var(--gray-50);border-left-color:var(--gray-200);">
                            @if($isOK) <span class="ir-chip-ok">✓ Disetujui</span>
                            @elseif($isRej) <span class="ir-chip-rej">✗ Ditolak</span>
                                @if($det?->hsse_catatan)
                                <div style="font-size:.67rem;color:#b91c1c;margin-top:4px;font-style:italic;">{{ $det->hsse_catatan }}</div>
                                @endif
                            @else <span style="font-size:.68rem;color:var(--gray-400);">Belum direview</span>
                            @endif
                        </div>
                    @endif
                </td>
            ...
            <tr class="kpi-total-row">
                ...
                <td colspan="{{ ($isHsse || $isSA) ? 2 : 1 }}" style="padding:.5rem;">
                    <div class="score-badges">
                        <div class="score-sub lag">
                            <span class="lbl">Lagging</span>
                            <span class="val" style="color:#fde68a;" id="t-lag">{{ number_format($tsLag,2) }}</span>
                        </div>
                        <div style="font-size:1.45rem;font-weight:800;color:#fff;" id="t-total">{{ number_format($ts,2) }}</div>
                        <div class="score-sub lead">
                            <span class="lbl">Leading</span>
                            <span class="val" style="color:#bfdbfe;" id="t-lead">{{ number_format($tsLead,2) }}</span>
                        </div>
                    </div>
                </td>
            </tr>
    ...
    <div class="step-nav">
        ...
        <div style="display:flex;gap:.6rem;align-items:center;">
            ...
            @endif
            @if(($canEditHsse || $isSA) && $isEdit)
            <a href="{{ route('kpi-hsse.show', $report) }}" class="btn-step btn-step-primary" style="text-decoration:none;">👁 Lihat Detail</a>
            @endif
        </div>
    </div>
</div>{{-- /step-2 --}}
...
<div class="kpi-sidebar-inner">
    ...
    <div class="sidebar-card">
        ...
        <div style="display:flex;align-items:center;gap:.9rem;margin-bottom:.85rem;">
            <div class="score-ring {{ $ringCls }}" style="font-size:.88rem;" id="sb-ring">
                <span id="sb-val">{{ $ts > 0 ? number_format($ts,1) : '—' }}</span>
                <span style="font-size:.48rem;opacity:.65;">/100</span>
            </div>
            <div style="flex:1;">
                <div style="display:flex;justify-content:space-between;margin-bottom:.35rem;">
                    <span style="font-size:.68rem;color:var(--gray-400);">Lagging</span>
                    <strong id="sb-lag" style="font-size:.82rem;color:var(--amber);">{{ number_format($tsLag,2) }}</strong>
                </div>
                <div style="display:flex;justify-content:space-between;">
                    <span style="font-size:.68rem;color:var(--gray-400);">Leading</span>
                    <strong id="sb-lead" style="font-size:.82rem;color:var(--blue);">{{ number_format($tsLead,2) }}</strong>
                </div>
            </div>
        </div>
        <div class="mini-prog">
            <div class="mini-prog-fill" id="sb-bar" style="width:{{ min($ts,100) }}%;"></div>
        </div>
        @if(!($isHsse || $isSA))
    ...
    @if(!$isCreate && ($isHsse || $isSA) && $totalItems > 0)
    @php
    ...
    <div class="sidebar-card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.5rem;text-align:center;margin-bottom:.75rem;">
            <div>
                <div style="font-weight:800;color:var(--green);font-size:1.35rem;" id="cnt-ap">{{ $apCnt }}</div>
                <div style="font-size:.63rem;color:var(--gray-400);">✓ Disetujui</div>
            </div>
            <div>
                <div style="font-weight:800;color:var(--red);font-size:1.35rem;" id="cnt-rj">{{ $rjCnt2 }}</div>
                <div style="font-size:.63rem;color:var(--gray-400);">✗ Ditolak</div>
            </div>
        </div>
        <div class="mini-prog mb-2">
            <div class="mini-prog-fill" id="rv-prog-fill" style="width:{{ $rvPct }}%;"></div>
        </div>
        <div style="font-size:.69rem;color:var(--gray-400);text-align:center;margin-bottom:.5rem;" id="rv-lbl">
            {{ $rvDone }} / {{ $totalItems }} direview
        </div>
    </div>
    @endif
    ...
    <div class="sidebar-card">
        ...
        <div style="display:grid;gap:.5rem;">
            ...
            @elseif($canEditHsse || ($isSA && !$isCreate))
            ...
            @else
                @if($report && $canEditKoord)
                <a href="{{ route('kpi-hsse.edit', $report) }}" class="btn btn-outline-primary btn-sm rounded-pill w-100">✏ Edit</a>
                @endif
                @if($report && ($isSA || ($isHsse && in_array($rStatus, ['submitted','validated']))))
                <a href="{{ route('kpi-hsse.edit', $report) }}" class="btn btn-outline-primary btn-sm rounded-pill w-100">✏ Edit Penilaian</a>
                @endif
            @endif
        </div>
    </div>
    ...
    @if(!$isCreate)
    <div class="sidebar-card">
        <div class="sb-title">Kapal / Unit ({{ $report->vessels->count() }})</div>
        @foreach($report->vessels->sortBy('sort_order') as $v)
        <div class="vessel-chip">
            <div class="vessel-chip-name">
                {{ $v->vessel_name }}
                @if($v->vessel_count)<span style="background:rgba(29,111,232,.1);color:#1e40af;border-radius:4px;padding:1px 6px;font-size:.66rem;font-weight:700;margin-left:3px;">{{ $v->vessel_count }}</span>@endif
            </div>
            @if($v->contract_number)<div class="vessel-chip-meta">📄 {{ $v->contract_number }}</div>@endif
            @if($v->contract_end_date)
            @php $exp = $v->contract_end_date->isPast(); @endphp
            <div class="vessel-chip-meta" style="{{ $exp?'color:#dc2626;':'' }}">
                📅 s/d {{ $v->contract_end_date->format('d M Y') }}
                @if($exp) ⚠ Expired @endif
            </div>
            @endif
        </div>
        @endforeach
    ...
    @if(!$isCreate && $report->statusLogs->count())
    <div class="sidebar-card">
        ...
        <div style="display:flex;gap:.65rem;margin-bottom:.9rem;">
            ...
            <div style="flex:1;">
                <div style="display:flex;justify-content:space-between;align-items:baseline;">
                    <span style="font-weight:600;font-size:.81rem;">{{ ucfirst($log->to_status) }}</span>
                    <span style="font-size:.66rem;color:var(--gray-400);">{{ $log->acted_at->format('d M Y') }}</span>
                </div>
                <div style="font-size:.71rem;color:var(--gray-400);">{{ $log->actedBy->name ?? '-' }}</div>
                @if($log->comment)
                <div style="font-size:.68rem;color:var(--gray-400);font-style:italic;padding-left:7px;border-left:2px solid var(--gray-200);margin-top:2px;">"{{ Str::limit($log->comment,65) }}"</div>
                @endif
            </div>
        </div>
        @endforeach
...
<div class="sticky-bar">
    ...
    <div style="display:flex;gap:.5rem;">
        <a href="{{ route('kpi-hsse.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill px-3">Batal</a>
        <button type="button" class="btn btn-primary fw-bold rounded-pill px-4" onclick="doSaveDraft()">💾 Simpan Draft</button>
    </div>
</div>
@elseif($canEditKoord && ($isEdit || $isShow))
<div class="sticky-bar">
    <div class="bar-info">
        <strong>{{ $report->company->name ?? '-' }}</strong> — {{ $report->kpiPeriod->label ?? '-' }}
        <span class="autosave-badge ms-2">✅ Auto-save aktif</span>
    </div>
    <div style="display:flex;gap:.5rem;">
        <a href="{{ route('kpi-hsse.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill px-3">← Daftar</a>
...
<div class="sticky-bar">
    <div class="bar-info">
        Penilaian HSSE — <strong>{{ $report->kpiPeriod->label ?? '-' }}</strong>
        <span class="badge badge-{{ $rStatus }} ms-2">{{ ['draft'=>'Draft','submitted'=>'Submitted','validated'=>'Validated','rejected'=>'Rejected'][$rStatus] ?? ucfirst($rStatus) }}</span>
    </div>
    <div style="display:flex;gap:.5rem;">
        <a href="{{ route('kpi-hsse.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill px-3">← Daftar</a>
...
{{-- ══════ MODAL: Submit ══════ --}}
@if(!$isCreate && $report && in_array($rStatus,['draft','rejected']) && ($isKoord || $isSA))
<div class="modal fade" id="submitModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:var(--radius);border:none;box-shadow:var(--shadow-lg);">
            <form action="{{ route('kpi-hsse.submit', $report) }}" method="POST">
                @csrf
...
{{-- ══════ MODAL: Simpan Draft ══════ --}}
@if(!$isCreate && $report && ($canEditKoord || $canEditHsse || $isSA))
<div class="modal fade" id="draftModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content" style="border-radius:var(--radius);border:none;box-shadow:var(--shadow-lg);">
            <form id="form-save-draft" action="{{ route('kpi-hsse.draft', $report) }}" method="POST">
                @csrf @method('PUT')
                <div class="modal-header border-0 pb-0">
...
const SCORE_URL = '{{ isset($kpiReport) ? route("kpi-hsse.score.update", $kpiReport) : "" }}';
...
function buildDelUrl(evId) {
    return '{{ isset($kpiReport) ? url("kpi-hsse/".$kpiReport->id."/evidences") : "" }}/' + evId;
}
...
═══════════════════════════════════════════════════════════ */
@if(isset($canEditHsse) && $canEditHsse)
...
async function doAddVessel() {
    ...
    try {
        const r = await fetch('{{ isset($kpiReport) ? route("kpi-hsse.vessels.store",$kpiReport) : "" }}',
